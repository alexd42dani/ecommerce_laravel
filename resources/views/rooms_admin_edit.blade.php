      ...
      <div class="row">
        <div class="col-lg-7 mx-auto">
          <div class="p-5">
            ...
            @endif
            <form method="POST" class="user" id="form_login" enctype="multipart/form-data" action="{{route('rooms_admin.update',$result[0]->id)}}">
              @csrf
              @method('PUT')
              <div class="form-group">
                <input type="text" placeholder="Nombre" class="form-control form-control-user @error('title') is-invalid @enderror" id="title" name="title" value="{{$result[0]->title}}" required>
              </div>
              <div class="form-group">
                <textarea placeholder="Descripción" class="form-control form-control-user @error('description') is-invalid @enderror" id="description" name="description" required>{{$result[0]->description}}</textarea>
              </div>
              <div class="form-group">
                <input type="text" placeholder="Precio" class="form-control form-control-user @error('price') is-invalid @enderror" id="price" name="price" value="{{$result[0]->price}}" required>
              </div>
              <div class="form-group">
                <label for="myfile" style="padding-left:0.8rem">Selecciona una foto:</label>
                <input type="file" class="form-control form-control-user @error('photo') is-invalid @enderror" style="padding:1rem 0.5rem 2.3rem" id="photo" name="photo">
              </div>
              <div class="form-group">
                <label for="myfile" style="padding-left:0.8rem">Selecciona segunda foto:</label>
                <input type="file" class="form-control form-control-user @error('photo1') is-invalid @enderror" style="padding:1rem 0.5rem 2.3rem" id="photo1" name="photo1">
              </div>
              <div class="form-group">
              ...
              <div class="form-group">
                ...
                <select class="w-100 form-control" name="estado" id="estado">
                  <option value="">--Selecciona una opción--</option>
                  <option value="0" {{ $result[0]->status=="0" ? 'selected' : '' }}>Inactivo</option>
                  <option value="1" {{ $result[0]->status=="1" ? 'selected' : '' }}>Activo</option>
                </select>
              </div>
              <hr>
              <div class="form-group">
                    ...
                    <div class="col-12 my-4">
                      <div class="modal-body table-responsive">
                        <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                          ...
                          <tbody>
                          @foreach($chara as $charac)
                            <tr>
                                <td>{{$charac->id_characteristic}}</td>
                                <td>{{$charac->name}}</td>
                                <td><button type="button" class="btn btn-primary mx-auto" id="delete_row">Quitar</button>
                                    <input type="hidden" name="input[]" value={{ $charac->id_characteristic }}>
                                </td>
                            </tr>
                            @endforeach
                          </tbody>
